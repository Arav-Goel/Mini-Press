...
| **bcrypt** / **argon2** (password hashing) | `node:crypto` → `scryptSync` | `src/auth.js` | scrypt is memory-hard and purpose-built for password storage — the honest stdlib answer per the cheat sheet's own guidance ("scrypt and pbkdf2_hmac are in every stdlib here. Argon2 is not"). Composed correctly: random salt per user, `timingSafeEqual` for comparison, never a bare hash comparison with `===`. |
